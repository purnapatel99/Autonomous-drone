...
float velocityZ, velocityX, velocityY, vZ, vX, vY, thrust, trust_const;
int pwm_value1, pwm_value2, pwm_value3, pwm_value4, Roll=93, Yaw=93, Pitch=93, Aux1, Aux2=40, Throttle=45;
...
void loop() { 
  ...
  velocityZ = (velocityZ*trust_const)+((1-trust_const)*vZ);
  velocityX = ((distanceX - aX)*1000)/t2;
  velocityX = (velocityX*trust_const)+((1-trust_const)*vX);
  velocityY = ((distanceY - aY)*1000)/t2;
  velocityY = (velocityY*trust_const)+((1-trust_const)*vY);
  aZ = distanceZ;
  aX = distanceX;
  aY = distanceY;
  vZ = velocityZ;
